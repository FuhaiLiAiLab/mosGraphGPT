...
def arg_parse():
    ...
    parser.add_argument('--dropout', dest = 'dropout', type = float,
                help = 'Dropout rate.')
    parser.add_argument('--train_result_path', nargs='?', dest='train_result_path', default='gat', 
                        help='save path for model result. (default: pretrain+gformer)')
    parser.add_argument('--fold_n', dest = 'fold_n', type = int, default = 1, 
                        help = 'fold number for training. (default: 1)')
...
def build_geogat_model(args, device, graph_output_folder, num_class):
    ...
    # [num_edge]
    gene_num_edge_df = pd.read_csv(os.path.join(graph_output_folder, 'all-gene-edge-num.csv'))
...
def train_geogat(args, fold_n, load_path, iteration_num, device, graph_output_folder, num_class):
    ...
    for i in range(1, epoch_num + 1):
        ...
        # fold_n = 1
        test_save_path = path
        test_acc, test_loss, tmp_test_input_df = test_geogat(args, fold_n, model, test_save_path, device, graph_output_folder, i)
...
if __name__ == "__main__":
    ...
    epoch_iteration = int(dl_input_num / args.batch_size)
    start_iter_num = args.num_epochs * epoch_iteration
    train_geogat(args, fold_n, load_path, start_iter_num, device, graph_output_folder, num_class)