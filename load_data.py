...
class UCSC_LoadData():
    ...
    def load_batch(self, index, upper_index, place_num, graph_output_folder, processed_dataset):
        ...
        num_gene_epi = final_meth_core_promoter_df.shape[0]
        num_gene_tran = final_rnaseq_df.shape[0]
        num_gene_mut = final_mut_df.shape[0]
        num_gene_prot = final_proteomics_df.shape[0]
        ...
        for row in split_input_df.iloc[index : upper_index].itertuples():
            ...
            for row in final_annotation_gene_df.itertuples():
                ...
                if node_type == 'Gene-Epi':
                    ...
                    x_input_list += [0.0] * 3
                elif node_type == 'Gene-Geno':
                    x_input_list += [0.0] * 5
                    x_input_list.append(gene_mut_list[i - num_gene_epi])
                    x_input_list += [0.0] * 2
                elif node_type == 'Gene-Tran':
                    x_input_list += [0.0] * 6
                    x_input_list.append(gene_rna_list[i - num_gene_epi - num_gene_mut])
                    x_input_list += [0.0] * 1
                elif node_type == 'Gene-Prot':
                    x_input_list += [0.0] * 7
                    x_input_list.append(gene_proteomics_list[i - num_gene_epi - num_gene_mut - num_gene_tran])
    ...
    def load_ppi_adj_edgeindex(self, graph_output_folder):
        form_data_path = './' + graph_output_folder + '/form_data'
        # Form a whole adjacent matrix
        ...
        for i in range(len(src_gene_list)):
            ...
            adj[col_idx, row_idx] = 1 # whether we want ['sym']
        adj_sparse = sparse.csr_matrix(adj)
        sparse.save_npz(form_data_path + '/ppi_adj_sparse.npz', adj_sparse)
        # [edge_index]
        source_nodes, target_nodes = adj_sparse.nonzero()
        ppi_edge_index = torch.tensor([source_nodes, target_nodes], dtype=torch.long)
        np.save(form_data_path + '/ppi_edge_index.npy', ppi_edge_index)
...
class ROSMAP_LoadData():
    ...
    def load_batch(self, index, upper_index, place_num, graph_output_folder, processed_dataset):
        ...
        num_gene_epi = final_meth_core_promoter_df.shape[0]
        num_gene_tran = final_rnaseq_df.shape[0]
        num_gene_mut = final_cnv_df.shape[0]
        num_gene_prot = final_proteomics_df.shape[0]
        print('READING FINISHED ...')
        # Combine a batch size as [x_batch, y_batch]
        print('-----' + str(index) + ' to ' + str(upper_index) + '-----')
        ...
        for row in split_input_df.iloc[index : upper_index].itertuples():
            ...
            for row in final_annotation_gene_df.itertuples():
                ...
                if node_type == 'Gene-Epi':
                    ...
                    x_input_list += [0.0] * 5
                    x_input_list.append(gene_cnv_list[i - num_gene_epi])
                    x_input_list += [0.0] * 2
                elif node_type == 'Gene-Tran':
                    x_input_list += [0.0] * 6
                    x_input_list.append(gene_rna_list[i - num_gene_epi - num_gene_mut])
...
if __name__ == "__main__":
    ...
    if dataset == 'UCSC':
        ...
        UCSC_LoadData().combine_whole_dataset(k, graph_output_folder)
    elif dataset == 'ROSMAP':
    ...
    if dataset == 'UCSC':
        ...
        UCSC_LoadData().load_internal_adj_edgeindex(graph_output_folder)
        UCSC_LoadData().load_ppi_adj_edgeindex(graph_output_folder)
    elif dataset == 'ROSMAP':
        ROSMAP_LoadData().load_adj_edgeindex(graph_output_folder)
        ROSMAP_LoadData().load_internal_adj_edgeindex(graph_output_folder)
        ROSMAP_LoadData().load_ppi_adj_edgeindex(graph_output_folder)
