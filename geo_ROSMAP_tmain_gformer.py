...
def arg_parse():
    ...
    parser.add_argument('--dropout', dest = 'dropout', type = float,
                help = 'Dropout rate.')
    parser.add_argument('--train_result_path', nargs='?', dest='train_result_path', default='gformer', 
                        help='save path for model result. (default: gformer)')
...
def build_geogformer_model(args, device, graph_output_folder, num_class):
    ...
    model = GraphFormerDecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
                    num_node=num_node, num_head=args.num_head, device=device, num_class=num_class)
...
def train_geogformer(args, fold_n, load_path, iteration_num, device, graph_output_folder, num_class):
    ...
    for i in range(1, epoch_num + 1):
        ...
        # fold_n = 1
        test_save_path = path
        test_acc, test_loss, tmp_test_input_df = test_geogformer(args, fold_n, model, test_save_path, device, graph_output_folder, i)
...
if __name__ == "__main__":
    ...
    train_geogformer(args, fold_n, load_path, start_iter_num, device, graph_output_folder, num_class)